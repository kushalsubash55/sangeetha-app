...
create or replace function public.record_delivery_payment(
  ...
  p_upi_recipient text default null
)
returns table (
  order_id uuid,
  bill_number text,
...
begin
  ...
  if p_payment_method = 'upi' and nullif(btrim(coalesce(p_upi_recipient, '')), '') is null then
    raise exception 'Choose UPI recipient.';
  end if;
  ...
  values (
    ...
    p_payment_method,
    v_payment_type,
    case when p_payment_method = 'upi' then nullif(btrim(coalesce(p_upi_recipient, '')), '') else null end,
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'upi_recipient', case when p_payment_method = 'upi' then nullif(btrim(coalesce(p_upi_recipient, '')), '') else null end,
